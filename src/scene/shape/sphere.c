...
/*   Updated: 2020/07/28 21:34:45 by wkorande         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		intersects_sphere(t_ray *ray, t_shape *sphere, t_raycast_hit *hit)
{
	...
	if (sphere->material->explode > EPSILON)
		temp = sphere->radius + sphere->material->explode * ft_inv_lerp_d((double)rand(), 0, RAND_MAX);
	else
		temp = sphere->radius;
